...
@Composable
fun MetaLoginScreen(backStack: NavBackStack<Route>) {
    val state by MetaClient.state.collectAsState()
    ...
    Column(
        ...
        Text(
            text = "To connect Messenger, you need to extract cookies from your browser after logging in to messenger.com",
        ...
        Text(
            text = "Required cookies: c_user, xs, datr",
        ...
        Text(
            text = "Format: {\"c_user\":\"123\",\"xs\":\"abc\",...} or c_user=123; xs=abc; ...",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant
        )
        ...
        when (val currentState = state) {
            is MetaClient.State.NeedsSetup -> {
                Button(
                    onClick = {
                        isValidating = true
                        errorMessage = null
                        val cookies = parseCookies(cookiesText)
                        if (cookies.isNotEmpty() && cookies.containsKey("c_user")) {
                            MetaClient.saveAuthData(cookies)
                        } else {
                            errorMessage = "Invalid cookies. Make sure c_user is present."
                            isValidating = false
                        }
                    },
                    modifier = Modifier.fillMaxWidth(),
            ...
            is MetaClient.State.Connecting -> {
                CircularProgressIndicator()
                Spacer(modifier = Modifier.height(8.dp))
                Text(
            ...
            is MetaClient.State.Disconnected -> {
                Text(
        ...
        Text(
            text = "How to get cookies:\n" +
                    "1. Open messenger.com in your browser and log in\n" +
                    "2. Open Developer Tools (F12)\n" +
                    "3. Go to Application/Storage > Cookies\n" +
                    "4. Copy the values for c_user, xs, datr, sb, fr, wd\n" +
